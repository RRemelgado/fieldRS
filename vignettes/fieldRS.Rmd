...
``` {r echo=FALSE}
kable_styling(kable(head(corrected.crop$label.count, 3), format="html", align="c", full_width=TRUE), "stripped", bootstrap_options="responsive") # label frequency
plot(corrected.crop$label.count.plot) # show label frequency plot
```
...
### Finding sampling mistakes through machine-learning
For each class, the function identified the corresponding polygons and iterated through each polygon keeping it for validation while using the remaining ones for training. Then, the function recorded if the class of this polygon was succefully predicted. Finally, for each class, the function derived an aggregated F1-score - based on all true and false positives and negatives - and reported on the accuracy of each polygon. Below we can see the results of this anlysis. polygons colored red in the plot weren't succefully predicted by the classifier.
...
plot(fieldData)
plot(fieldData[!fieldData$validation,], col="red", add=TRUE)
```
